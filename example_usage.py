...
from dataset_util import get_random_wsi_patch
...
def input_setup(batch_size=32, dataset_dir='./path_to_folder_with_WSIs', wsi_ext='.svs'):
    ...
    wsi_dataset = path_ds.map(lambda filename: tf.py_function(get_random_wsi_patch, [filename], tf.float32), num_parallel_calls=40)
